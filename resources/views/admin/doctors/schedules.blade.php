<x-admin-layout :breadcrumbs="[
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 dark:bg-gray-800">
        <form id="schedule-form" action="{{ route('admin.doctors.schedules.store', $doctor) }}" method="POST">
            @csrf

            <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Horarios</h1>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Gestor de horarios de {{ $doctor->user->name ?? 'N/A' }}</p>
                </div>
                    ...
                    <thead class="text-xs text-gray-400 uppercase bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                        <tr>
                            ...
                            @foreach($days as $dayK => $dayV)
                                <th scope="col" class="px-2 py-3 font-semibold pb-4">{{ strtoupper($dayV) }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('schedule-form').addEventListener('submit', function(e) {
                ...
                groupedAvailabilities.forEach(function(avail, index) {
                    const i1 = document.createElement('input');
                    i1.type = 'hidden';
                    i1.name = 'availabilities[' + index + '][day_of_week]';
                    i1.value = avail.day_of_week;
                    form.appendChild(i1);
                    const i2 = document.createElement('input');
                    i2.type = 'hidden';
                    i2.name = 'availabilities[' + index + '][start_time]';
                    i2.value = avail.start_time;
                    form.appendChild(i2);
                    const i3 = document.createElement('input');
                    i3.type = 'hidden';
                    i3.name = 'availabilities[' + index + '][end_time]';
                    i3.value = avail.end_time;
                    form.appendChild(i3);
                });
